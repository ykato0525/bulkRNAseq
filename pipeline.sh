...
# サイトの中からfastqファイルをコピーする (20240124)
...
#export PATH=~/RNAseq/tools/bin:$PATH
source ~/.bashrc
# エラー回避のための上限を挙げておく
ulimit -n 16000

# 変数の設定
STAR_REF=~/Desktop/RNAseq/ref/STAR_index
RSEM_REF=~/Desktop/RNAseq/ref/RSEM_Reference
...
# gene expression TPM
cut -f 1,$(awk 'NR==1 {for (i=1; i<=NF; i++) if ($i ~ /TPM/) printf("%d,",i)}' genes.join.txt | sed 's/,$//') genes.join.txt | sed "1s/_TPM//g" > genes_TPM.txt

# gene expression expected count
cut -f 1,$(awk 'NR==1 {for (i=1; i<=NF; i++) if ($i ~ /expected_count/) printf("%d,",i)}' genes.join.txt | sed 's/,$//') genes.join.txt | sed "1s/_expected_count//g" > genes_expected_count.txt

# transcript expression TPM
cut -f 1,$(awk 'NR==1 {for (i=1; i<=NF; i++) if ($i ~ /TPM/) printf("%d,",i)}' isoforms.join.txt | sed 's/,$//') isoforms.join.txt | sed "1s/_TPM//g" > isoforms_TPM.txt

# transcript expression expected count
cut -f 1,$(awk 'NR==1 {for (i=1; i<=NF; i++) if ($i ~ /expected_count/) printf("%d,",i)}' isoforms.join.txt | sed 's/,$//') isoforms.join.txt | sed "1s/_expected_count//g" > isoforms_expected_count.txt
